...
if [ "$XDG_CURRENT_DESKTOP" = "Hyprland" ]; then
  ...
  RULE='
-- alansare_overlay no_blur windowrule
hl.window_rule({
    match = {
        title = "^(alansare_overlay)$",
    },
    no_blur = true,
})'
  ...
  if grep -Fq 'title = "^(alansare_overlay)$"' "$LUA_FILE"; then
    echo "alansare_overlay no_blur windowrule has already been added, skipping"
  else
    echo "$RULE" >> "$LUA_FILE"
